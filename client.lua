...
local function addRadialItem(radialData)
    ...
    radialMenu.ItemRadial[#radialMenu.ItemRadial + 1] = radialData
end
...
local function removeRadialItem(radialId)
    ...
    for k, v in pairs(radialMenu.ItemRadial) do
        if v.id == radialId then
            table.remove(radialMenu.ItemRadial, k)
        end
    end
end
...
CreateThread(function()
    ...
    while true do
        ...
        if IsControlPressed(1, Keys[keyBind]) and GetLastInputMethod(2) then
            ...
            for num, menuConfig in ipairs(radialMenu.ItemRadial) do
...
AddEventHandler('onClientResourceStop', function(resource)
    for i = #radialMenu.ItemRadial, 1, -1 do
        local item = radialMenu.ItemRadial[i]
        if item.resources == resource then
            table.remove(radialMenu.ItemRadial, i)
        end
    end
end)
